  ...
  switch (result.type) {
    case "banner": {
      ...
      if (processedData.header) {
        processedData.header = renderMarkdown(processedData.header, true);
      }

      if (processedData.content) {
        processedData.content = renderMarkdown(processedData.content);
      }

      if (processedData.footer) {
        processedData.footer = renderMarkdown(processedData.footer, true);
      }
    ...
    case "profile": {
      ...
      if (processedData.slogan) {
        processedData.slogan = renderMarkdown(processedData.slogan);
      }
